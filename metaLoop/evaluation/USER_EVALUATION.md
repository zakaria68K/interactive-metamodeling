...
# State Machine Metamodel Quiz
 
15 multiple-choice questions. Each question has exactly 3 answer choices (A, B, C).
Answer key is at the bottom — do not consult it before completing the quiz.
 
---
 
## Questions
 
**1.** In a state machine metamodel, how is the starting point of execution typically represented?
...
**2.** A `State` has three action slots: entry, exit, and `doActivity`. When does the `doActivity` action execute?
...
**3.** In a metamodel, what is the difference between a *containment* reference and a regular *reference*?
...
**4.** A `Transition` class has both a `trigger` attribute of type String and a `triggerEvent` reference pointing to an `Event` class. What problem does this create?
...
**5.** Which multiplicity is correct for the `states` reference in `StateMachine`, and why?
...
**6.** What does it mean when a `Transition` has no event and no guard set?
...
**7.** Why might a `Guard` be modeled as a separate class with an `expression` attribute, rather than just a String attribute directly on `Transition`?
...
**8.** A `State` has a containment reference `subStates` pointing to other `State` instances. What kind of structure does this enable?
...
**9.** In a state machine metamodel, `source` and `target` on `Transition` are typed as `State`. What scenario does this prevent?
...
**10.** A validator flags: *"redundancy between `trigger` attribute and `triggerEvent` reference."* What is the recommended fix?
...
**11.** What is the purpose of entry and exit actions on a `State`?
...
**12.** A `StateMachine` uses a regular *reference* (not containment) to link to its `State` instances. What risk does this introduce?
...
**13.** In the metamodel, `Action` has a `code` attribute of type String. What is the main limitation of this for a tool that needs to execute or analyse the action?
...
**14.** A participant generates a metamodel where `Guard` has a `relatedTransition` reference back to `Transition`, and `Transition` already has a `guard` reference to `Guard`. What issue does this raise?
...
**15.** After generating each concept chunk, the system validates the metamodel against a sample instance. What is the purpose of this validation step?
